...
可选族覆盖 plain 访问（SB/LB/MP/MPTSO/CoRR/CoWR/CoRW/CoWW/WRC/IRIW/2+2W）、原子（AMO 压原子性、LRSC 压预留冲突）与依赖扰动（LBdep：在 LB 拓扑里插 in-window 数据依赖链/延迟）。相干/序族（CoWR/CoRW/CoWW/2+2W）值敏感，builder 自动取互异竞争值；LBdep 的延迟链长在 `--randomize` 下从 [1,8] 抽取。原子族用 AMO/LR/SC 事件，是 `--test-family` 的一等公民；它们额外受两条随机化轴作用（见下）。
